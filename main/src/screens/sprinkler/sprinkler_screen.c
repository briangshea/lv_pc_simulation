...
void calendar_start_changed_cb(lv_event_t *e) {
    ...
    if(cbdata->field == PROG_START_FIELD) {
        sprinkler_prog_set_start(activeProg, &newTime);
    }
    else if(cbdata->field == PROG_END_FIELD) {
        sprinkler_prog_set_end(activeProg, &newTime);
    }
...
void start_clicked_cb(lv_event_t *e) {
    ...
    struct tm *start_tm = localtime(sprinkler_prog_get_start(activeProg));
...
void screen_sprinkler_display(sprinkler_prog_t *selectedProgram) {
    ...
    lv_obj_add_event_cb(start, start_clicked_cb, LV_EVENT_CLICKED, NULL);
